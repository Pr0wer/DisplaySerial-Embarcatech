...
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y)
{
  ...
  if (c >= 'A' && c <= 'Z')
  {
    index = (c - 'A' + 1) * FONT_WIDTH; // Para letras maiúsculas
  }
  else if (c >= 'a' && c <= 'z')
  {
    index = (c - 'a' + 27) * FONT_WIDTH;
  }
  else  if (c >= '0' && c <= '9')
  {
    index = (c - '0' + 53) * FONT_WIDTH; // Adiciona o deslocamento necessário
  }
...
void ssd1306_draw_string(ssd1306_t *ssd, const char *str, uint8_t x, uint8_t y)
{
  while (*str)
  {
    ...
    x += FONT_WIDTH;
    if (x + FONT_WIDTH >= ssd->width)
    {
      x = 0;
      y += FONT_WIDTH;
    }
    if (y + FONT_WIDTH >= ssd->height)
    {
      break;
    }
  }
